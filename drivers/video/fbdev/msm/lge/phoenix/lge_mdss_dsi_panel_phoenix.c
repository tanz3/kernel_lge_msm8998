...
int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (pinfo->panel_dead) {
		for(extra_cmds_index=0 ; extra_cmds_index < ctrl->lge_extra.num_extra_cmds; extra_cmds_index++){
			if (!strcmp(ctrl->lge_extra.extra_cmds_array[extra_cmds_index].name, "on-with-apo"))
				break;
		}

		if (extra_cmds_index >= ctrl->lge_extra.num_extra_cmds) {
			pr_info("%s : there is no extra-cmds parsed. \n", __func__);
			on_cmds = &ctrl->on_cmds;
		} else {
			on_cmds = &ctrl->lge_extra.extra_cmds_array[extra_cmds_index].lge_dsi_cmds;
			pr_info("%s : 'lge,mdss-dsi-extra-command-on-with-apo' is set. \n", __func__);
		}

	} else {
		on_cmds = &ctrl->on_cmds;
		pr_info("%s : 'qcom,mdss-dsi-on-command' is set. \n", __func__);
	}
	...
	if (ctrl->lge_extra.cur_mp_mode != LGE_MP_NOR && ctrl->lge_extra.cur_mp_mode != LGE_MP_OFF)
...
int lge_ddic_ops_init(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pr_info("%s: set ddic_ops sw49410\n", __func__);
