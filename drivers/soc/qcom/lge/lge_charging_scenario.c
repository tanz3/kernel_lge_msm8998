...
void lge_monitor_batt_temp(struct charging_info req, struct charging_rsp *res)
{
	...
	if(charging_state == CHG_BATT_DECCUR_STATE){
		//Default DECCUR state charge current = 0.3C
		res->chg_current = req.chg_current_ma / DECCUR_CHG_CURRENT_DIVIDER;

		if(battemp_state <= CHG_BATTEMP_10_12) {
			//In Cool State, Safety Charge Current under 4.0 V is 500 mA.
			//LG Charging Scernario v2.1
			if(req.batt_volt >= safe_bat_thr) {
				safe_bat_thr = DC_IUSB_VOLTUV - DC_IBAT_VOLTUV_HIS;
				res->chg_current = SAFETY_VOLTAGE_IBAT;
				pr_err("new low temperature profile over 4V\n");
			} else {
				safe_bat_thr = DC_IUSB_VOLTUV;
				pr_err("new low temperature profile\n");
			}
		}
	}else{
